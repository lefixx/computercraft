...
function claymaker()
    while true do
        for i,v in pairs(strainer.list()) do
            ...
            if (not algalDrawer.list()[5] or (( not algalDrawer.list()[4]) or algalDrawer.list()[4].count<=100)) and (not washDepot.list()[1]) then   -- if clay is less that 100 pull sand and wash it
                print("low on clay")
                for i,v in pairs(strainer.list()) do
                    if v.count == 64 then
